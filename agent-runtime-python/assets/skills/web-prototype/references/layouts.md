...
**Paste-ready section skeletons.** Drop into your HTML `<main id="content">`. Don't write sections from scratch — pick the closest layout, paste, swap copy.

## Pre-flight (do this once before pasting anything)

1. **Read your CSS token system** through the end of the `<style>` block. Every class used below must exist there. If one is missing, add it to `<style>` rather than inlining it on each section.
...
## Class inventory (must exist in your CSS)
